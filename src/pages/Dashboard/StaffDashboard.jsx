...
const StaffDashboard = () => {
  ...
  const pendingPatients = patients.filter(p => 
    p.status === 'registered' || p.status === 'results_entered'
  );
  ...
  return (
    <div className="staff-dashboard">
      <div className="staff-header">
        ...
        <Button onClick={() => navigate('/patients/add')} icon={Plus} size="large">
          Add Patient
        </Button>
      </div>
      ...
      {/* Pending Patients */}
      <Card title="Pending Patients" className="patients-section">
              ...
              return (
                <div key={patient.id} className="patient-card" onClick={() => {
                  ...
                  <div className="patient-status">
                    <span className={`status-badge ${patient.status.replace('_', '-')}`}>
                      {patient.status.replace('_', ' ').toUpperCase()}
                    </span>
                    <Button size="small">
                      {patient.status === 'registered' ? 'Enter Results' : 'View'}
                    </Button>
                  </div>
                </div>
              );
      ...
      {/* Recent Completed */}
      <Card title="Recently Completed Reports" className="patients-section">
                ...
                <div key={patient.id} className="patient-card" onClick={() => navigate(`/patients/${patient.id}`)}>
                  ...
                  <div className="patient-status">
                    <p className="completed-time">
                      {patient.reportedAt ? new Date(patient.reportedAt).toLocaleString() : '-'}
                    </p>
                    <Button size="small" variant="secondary">View Report</Button>
                  </div>
                </div>
              );
            })}
