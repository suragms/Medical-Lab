...
import { generateCombinedInvoice } from '../../utils/profilePdfHelper';
import { getInvoiceBlob } from '../../utils/invoicePdfGenerator';
...
const Patients = () => {
  ...
  // NEW: Handle invoice reprint separately
  const handleReprintInvoice = async (visitId) => {
    const visit = visits.find(v => v.visitId === visitId);
    if (!visit) return;
    ...
    try {
      ...
      console.log('🧾 Reprinting invoice for visit:', visit.visitId);

      // Generate invoice only
      const result = await generateCombinedInvoice(visitData, allProfiles, {
        download: true,
        print: false
      });
      ...
      console.error('Invoice reprint error:', error);
      toast.error('Failed to reprint invoice: ' + error.message);
    }
  };

  // NEW: Share invoice via WhatsApp
  const handleShareInvoiceWhatsApp = async (visitId) => {
    const visit = visits.find(v => v.visitId === visitId);
    if (!visit) return;
    ...
    try {
      toast.loading('📱 Preparing invoice for WhatsApp...');
      ...
      const invoiceData = {
        ...
        subtotal: 0,
        finalTotal: 0,
        amountPaid: visitData.paymentStatus === 'paid' ? 0 : 0
      };

      // Group tests by profile for invoice items
      const groupedTests = {};
      ...
      invoiceData.finalTotal = totalAmount;
      invoiceData.amountPaid = visitData.paymentStatus === 'paid' ? totalAmount : 0;

      const pdfBlob = await getInvoiceBlob(invoiceData);
      const result = await shareViaWhatsApp(pdfBlob, patient.name, patient.phone);

      toast.dismiss();
      if (result.success) {
        toast.success(result.message || 'WhatsApp opened with invoice!');
      } else {
  ...
  // NEW: Share invoice via Email
  const handleShareInvoiceEmail = async (visitId) => {
    const visit = visits.find(v => v.visitId === visitId);
    if (!visit) return;
    ...
    try {
      toast.loading('📧 Preparing invoice for Email...');
      ...
      const pdfBlob = await getInvoiceBlob(invoiceData);
      const result = await shareViaEmail(pdfBlob, patient.email, patient.name);

      toast.dismiss();
      if (result.success) {
        toast.success(result.message || 'Email client opened with invoice!');
      } else {
        toast.error('Failed to share invoice via Email');
      }
    } catch (error) {
                      ...
                      <td className="text-center" data-label="Invoice">
                        {visit.invoiceGenerated ? (
                          <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', flexWrap: 'wrap' }}>
                            <button className="icon-btn-success" onClick={() => handleReprintInvoice(visit.visitId)} title="Re-Print Invoice">
                              <Printer size={14} />
                            </button>
                            <button
                              className="icon-btn-success"
                              onClick={() => handleShareInvoiceWhatsApp(visit.visitId)}
                            ...
                            {patient.email && (
                              <button
                                className="icon-btn"
                                onClick={() => handleShareInvoiceEmail(visit.visitId)}
                          ...
                          <span className="payment-badge unpaid">Not Generated</span>
                        )}
                      </td>
