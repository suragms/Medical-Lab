...
const PatientDetails = () => {
  ...
  const buildVisitDataForPdf = () => {
    let signingTechnician = null;
    if (visit.signing_technician_id) {
      signingTechnician = getTechnicians().find((t) => t.technicianId === visit.signing_technician_id);
    }
    return { ...visit, patient, signingTechnician };
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    setIsGenerating(true);
    try {
      const visitData = buildVisitDataForPdf();
      const profiles = getProfiles();
      const result = await generateCombinedLabReports(visitData, profiles, { download: true, print: false });
      if (result?.success) {
  ...
  const handlePrint = async () => {
    ...
    try {
      const visitData = buildVisitDataForPdf();
      const profiles = getProfiles();
      const result = await generateCombinedLabReports(visitData, profiles, { download: false, print: true });
      if (result?.success) {
        toast.success('Opening print dialog...');
      } else {
        toast.error('Failed to print: ' + (result?.error || 'Unknown error'));
      }
    } catch (error) {
  ...
  const handleWhatsAppShare = async () => {
    ...
    try {
      const visitData = buildVisitDataForPdf();
      const profiles = getProfiles();
      const result = await shareLabReportsOnlyViaWhatsApp(visitData, profiles, patient.phone);
      if (result?.success) {
        toast.success(result.message || 'Shared via WhatsApp');
      } else {
        toast.error(result?.error || result?.message || 'Failed to share via WhatsApp');
      }
    } catch (error) {
  ...
  const handleEmailShare = async () => {
    ...
    try {
      const visitData = buildVisitDataForPdf();
      const profiles = getProfiles();
      const result = await shareLabReportsOnlyViaEmail(visitData, profiles, patient.email);
      if (result?.success) {
        toast.success(result.message || 'Email opened with PDF ready to attach');
      } else {
        toast.error(result?.error || result?.message || 'Failed to share via email');
      }
    } catch (error) {
