...
export const generateLabReportsOnly = async (visitData, profiles = [], options = {}) => {
  ...
  // Generate Lab Reports for each profile (NO INVOICE PAGE)
  for (let i = 0; i < profileEntries.length; i++) {
    const [profileId, tests] = profileEntries[i];
    
    console.log(`🔍 Profile ${i + 1} - profileId:`, profileId);
    console.log(`🔍 Profile ${i + 1} - tests:`, tests);
    console.log(`🔍 Profile ${i + 1} - test values:`, tests.map(t => ({ name: t.name, value: t.value })));
    
    // Add new page for each profile after the first one
    if (i > 0) {
      doc.addPage();
      console.log(`➕ Added new page for profile ${i + 1}`);
    }
    
    const profile = profiles.find(p => p.profileId === profileId);
    const profileName = profile?.name || tests[0]?.profileName || 'Test Results';
    
    console.log(`📝 Adding profile ${i + 1}/${profileEntries.length}: ${profileName}`);
    console.log(`📄 Current PDF page count: ${doc.internal.getNumberOfPages()}`);
    ...
    console.log(`✅ Completed profile ${i + 1}, PDF now has ${doc.internal.getNumberOfPages()} pages`);
  }
  
  // Generate descriptive filename
  const patientName = (visitData.patient?.name || 'Patient').replace(/\s+/g, '_');
  const dateStr = new Date().toISOString().split('T')[0];
  const fileName = `LabReports_${patientName}_${visitData.visitId}.pdf`;
